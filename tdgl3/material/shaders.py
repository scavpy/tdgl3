...
class GLobject:
    """ an OpenGL object referenced by integer ID, and
    queryable with respect to integer values and a string
    error log """
...
class Shader(GLobject):
    ...
    def __init__(self, source, shadertype=GL_FRAGMENT_SHADER):
        if type(shadertype) is str:
            shadertype = {"vertex":GL_VERTEX_SHADER,
                          "fragment":GL_FRAGMENT_SHADER}[shadertype]
...
class ShaderProgram(GLobject):
    """ a shader program made of one or more shaders """
    ...
    def set(self, **uniforms):
        """ set uniform values by (keyword) name
            Only handles vec4 uniforms at the moment
        """
        for name, val in uniforms.items():
            num = self.uniforms.get(name)
            if num is None:
                nbuf = create_string_buffer(name.encode("ascii"))
                num = glGetUniformLocation(
                    self.id, cast(nbuf, POINTER(c_char)))
